...
`;
  ...
  generateAILayout(): void {
    ...
    if (shapes.length === 0) {
      alert('No components to arrange. Add some components first!');
      return;
    }
    
    console.log('🤖 AI Layout: Analyzing', shapes.length, 'components...');
    
    // Analyze components and categorize them
    const analysis = this.analyzeComponents(shapes);
    
    // Apply intelligent layout based on analysis
    this.applyIntelligentLayout(shapes, analysis);
    
    // Show summary
    this.showLayoutSummary(analysis);
    
    this.saveHistory();
    this.layer.batchDraw();
  }
  
  private analyzeComponents(shapes: any[]): any {
    const analysis = {
      databases: [] as any[],
      apis: [] as any[],
      services: [] as any[],
    ...
    shapes.forEach((shape: any) => {
      ...
      const category = shape.attrs.category || '';
      
      // Categorize by keywords and category
      if (category === 'storage' || name.includes('database') || name.includes('db') || 
          name.includes('sql') || name.includes('mongo') || name.includes('redis') ||
          name.includes('dynamodb') || name.includes('postgres')) {
        analysis.databases.push(shape);
      } else if (name.includes('api') || name.includes('gateway') || name.includes('endpoint')) {
        analysis.apis.push(shape);
      } else if (category === 'ai_models' || name.includes('gpt') || name.includes('model') || 
                 name.includes('ai') || name.includes('ml') || name.includes('bert') ||
                 name.includes('llama') || name.includes('claude')) {
        analysis.ai_models.push(shape);
        ...
        analysis.frontends.push(shape);
      } else if (category === 'infrastructure' || name.includes('server') || name.includes('vm') || 
                 name.includes('ec2') || name.includes('compute')) {
        analysis.infrastructure.push(shape);
      } else if (category === 'networking' || name.includes('network') || name.includes('vpc') || 
                 name.includes('subnet') || name.includes('router')) {
        analysis.networking.push(shape);
      } else if (name.includes('service') || name.includes('microservice') || name.includes('backend')) {
        analysis.services.push(shape);
      } else {
        analysis.others.push(shape);
      }
    });
    
    // Check for existing connections
    const connectors = this.layer.find('.connector');
    analysis.hasConnections = connectors.length > 0;
    
    // Suggest best layout based on analysis
    if (analysis.databases.length > 0 && (analysis.services.length > 0 || analysis.apis.length > 0)) {
      analysis.suggestedLayout = 'hierarchical'; // Bottom: DBs, Middle: Services, Top: UIs
    } else if (analysis.ai_models.length > 2) {
      analysis.suggestedLayout = 'grid'; // Grid for multiple AI models
    } else if (shapes.length > 10) {
      analysis.suggestedLayout = 'force'; // Force-directed for complex systems
    } else if (analysis.hasConnections) {
      analysis.suggestedLayout = 'hierarchical'; // Respect existing connections
    } else {
      analysis.suggestedLayout = 'circular'; // Default to circular
    }
    
    return analysis;
  }
  
  private applyIntelligentLayout(shapes: any[], analysis: any): void {
    ...
    console.log('📊 AI Analysis:', {
      ...
      frontends: analysis.frontends.length,
      ai_models: analysis.ai_models.length,
      infrastructure: analysis.infrastructure.length,
      networking: analysis.networking.length,
      others: analysis.others.length,
      suggestedLayout: analysis.suggestedLayout
    });
    
    // Apply hierarchical layout for typical architecture patterns
    if (analysis.suggestedLayout === 'hierarchical') {
      let currentY = padding;
      
      // Layer 1 (Top): Frontends/UIs
      if (analysis.frontends.length > 0) {
        this.arrangeInRow(analysis.frontends, padding, currentY, canvasWidth - padding * 2, spacing);
        currentY += 200;
      }
      
      // Layer 2: APIs/Gateways
      if (analysis.apis.length > 0) {
        this.arrangeInRow(analysis.apis, padding, currentY, canvasWidth - padding * 2, spacing);
        currentY += 200;
      }
      
      // Layer 3: AI Models
      if (analysis.ai_models.length > 0) {
        this.arrangeInRow(analysis.ai_models, padding, currentY, canvasWidth - padding * 2, spacing);
        currentY += 200;
      }
      
      // Layer 4: Services/Microservices
      if (analysis.services.length > 0) {
        this.arrangeInRow(analysis.services, padding, currentY, canvasWidth - padding * 2, spacing);
        currentY += 200;
      }
      
      // Layer 5: Infrastructure
      if (analysis.infrastructure.length > 0) {
        this.arrangeInRow(analysis.infrastructure, padding, currentY, canvasWidth - padding * 2, spacing);
        currentY += 200;
      }
      
      // Layer 6: Networking
      if (analysis.networking.length > 0) {
        this.arrangeInRow(analysis.networking, padding, currentY, canvasWidth - padding * 2, spacing);
        currentY += 200;
      }
      
      // Layer 7 (Bottom): Databases/Storage
      if (analysis.databases.length > 0) {
        this.arrangeInRow(analysis.databases, padding, currentY, canvasWidth - padding * 2, spacing);
        currentY += 200;
      }
      
      // Layer 8: Others
      if (analysis.others.length > 0) {
        this.arrangeInRow(analysis.others, padding, currentY, canvasWidth - padding * 2, spacing);
      }
    } else if (analysis.suggestedLayout === 'grid') {
      // Grid layout for AI models or uniform components
      this.applyGridLayout(shapes);
    } else if (analysis.suggestedLayout === 'force') {
      // Force-directed for complex systems
  ...
  private arrangeInRow(shapes: any[], startX: number, y: number, availableWidth: number, spacing: number): void {
    const totalSpacing = (shapes.length - 1) * spacing;
    const totalWidth = shapes.reduce((sum: number, shape: any) => sum + (shape.width() || 120), 0) + totalSpacing;
    
    let currentX = startX + (availableWidth - totalWidth) / 2; // Center the row
    
    shapes.forEach((shape: any) => {
      const width = shape.width() || 120;
      shape.x(currentX + width / 2);
      shape.y(y);
      currentX += width + spacing;
    });
  }
  
  private showLayoutSummary(analysis: any): void {
    const messages = [];
    
    if (analysis.databases.length > 0) {
      messages.push(`📦 ${analysis.databases.length} database(s) positioned at bottom layer`);
    }
    if (analysis.apis.length > 0) {
      messages.push(`🌐 ${analysis.apis.length} API(s) positioned in middle layer`);
    }
    if (analysis.services.length > 0) {
      messages.push(`⚙️ ${analysis.services.length} service(s) organized`);
    }
    if (analysis.frontends.length > 0) {
      messages.push(`🖥️ ${analysis.frontends.length} frontend(s) positioned at top`);
    }
    if (analysis.ai_models.length > 0) {
      messages.push(`🤖 ${analysis.ai_models.length} AI model(s) arranged`);
    }
    if (analysis.infrastructure.length > 0) {
      messages.push(`🏗️ ${analysis.infrastructure.length} infrastructure component(s)`);
    }
    
    const summary = messages.length > 0 ? messages.join('\n') : 'Components arranged intelligently!';
    
    console.log('✨ AI Layout Complete!\n' + summary);
    alert(`✨ AI Layout Applied!\n\nLayout: ${analysis.suggestedLayout.toUpperCase()}\n\n${summary}`);
  }
